...
function d_findVariable(s, i, len, lastType, jobstate)
	...
	while true do
		--	Stop at the end. Throw an error. This function MUST NOT meet the end not 
		if i > len then
		...
		--	{ means the start of a table not 
		elseif c == "{" then
			lastType = "table"
			typeRead = true
		...
		if sub(s, i, i) == "#" then
			...
			if e then
				...
				if id then
					...
					error("vON: Malformed table not  Reference ID starting at char #" .. i .. " doesn't contain a number not ")
				end
			else
				error("vON: Malformed table not  Cannot find end of reference ID start at char #" .. i .. " not ")
			end
		end
...
--	Just a number which points to a table.
	["table_reference"] = function(s, i, len, unnecessaryEnd, jobstate)
		...
		if a then
			...
			if n then
				return jobstate[1][n] or error("vON: Table reference does not point to a (yet) known table not "), a - 1
			else
				error("vON: Table reference definition does not contain a valid number not ")
			end
		end
...
--	Has various endings: \n, }, ~, : and ;, some of which will force the table deserializer to go one char backwards.
	["number"] = function(s, i, len, unnecessaryEnd, jobstate)
		...
		if a then
			return tonumber(sub(s, i, a - 1)) or error("vON: Number definition does not contain a valid number not "), a - 1
		end
